...
# Fungsi: Setup otomatis penuh, Ngrok ROBUST (dengan retry dan log), Auto Shutdown, Java Start.
...
auto_shutdown_timer() {
    # Pastikan screen command tersedia di sini
...
install_dependencies() {
    ...
    # Menambahkan pengecekan yang lebih ketat jika ada paket yang gagal terinstal
...
create_config_files() {
    ...
    # Menyalin konfigurasi dari file referensi (book.txt, EssentialsX, GriefPrevention)
...
start_server() {
    ...
    # Menjalankan Ngrok di latar belakang dan menyimpan error ke log
    ./ngrok start --all --config ngrok.yml > ngrok.log 2>&1 &
    
    echo "   Memberi waktu 10 detik agar Ngrok API stabil (Anti-Error Timeout)..."
    sleep 10 
    ...
    if [ -z "$IP_ADDRESS" ]; then
        ...
        echo "   CEK: Token Ngrok di start.sh valid (atau Ngrok Rate Limit)."
...
Setelah Anda mengganti `start.sh` di folder Termux Anda, jalankan lagi dengan langkah yang sama:

```bash
# Hentikan proses lama jika masih berjalan
pkill -f start.sh 
pkill -f ngrok 

# Beri izin (hanya untuk jaga-jaga)
chmod +x start.sh
