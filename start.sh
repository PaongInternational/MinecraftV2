...
# Fungsi: Setup otomatis penuh, Random MOTD, Ngrok ROBUST, Auto Shutdown 12 jam, Java Start.

# --- KONFIGURASI NGROK DAN SERVER ---
# GANTI DENGAN TOKEN NGROK MILIK ANDA!
NGROK_TOKEN="33RKXoLi8mLMccvpBbo1LoN3fCg_4AEGykdpBZeXx2TFHaCQj" 
...
create_config_files() {
    ...
    # 4.1 Menyalin Buku Panduan dari file book.txt
    if [ -f book.txt ]; then
        cp book.txt $PLUGIN_DIR/Essentials/book.txt
    else
        echo "   [PERINGATAN] File book.txt tidak ditemukan. Buku Panduan tidak akan terisi."
    fi

    # 4.2 Menyalin EssentialsX Config dari file EssentialsX_config_reference.yml
    if [ -f EssentialsX_config_reference.yml ]; then
        cp EssentialsX_config_reference.yml $PLUGIN_DIR/Essentials/config.yml
    else
        echo "   [PERINGATAN] File EssentialsX_config_reference.yml tidak ditemukan."
    fi

    # 4.3 Menyalin GriefPrevention Config dari file GriefPrevention_config_reference.yml
    if [ -f GriefPrevention_config_reference.yml ]; then
        mkdir -p $PLUGIN_DIR/GriefPrevention
        cp GriefPrevention_config_reference.yml $PLUGIN_DIR/GriefPrevention/config.yml
    else
        echo "   [PERINGATAN] File GriefPrevention_config_reference.yml tidak ditemukan."
    fi

    # 4.4 Crazy Crate - Dungeon.yml (Dibuat langsung di sini agar selalu ada)
...
initial_setup_download() {
    ...
    if [ ! -d $PLUGIN_DIR ]; then
        ...
        mkdir -p $PLUGIN_DIR
        wget -q -O $PLUGIN_DIR/Vault.jar "https://github.com/Vault/Vault/releases/download/1.7.3/Vault-1.7.3.jar" # FONDASI EKONOMI
...
# --- FUNGSI START UTAMA (NGROK ROBUST) ---
start_server() {
    display_banner
    echo "6. Mempersiapkan Ngrok..."
    ...
    while [ -z "$IP_ADDRESS" ] && [ $ATTEMPTS -lt 10 ]; do
        # Menggunakan --retry 5 untuk menjamin koneksi
        IP_ADDRESS=$(curl --silent --show-error --retry 5 --connect-timeout 5 http://127.0.0.1:4040/api/tunnels | grep -o 'tcp://[^"]*' | head -n 1)
    ...
    if [ -z "$IP_ADDRESS" ]; then
        ...
        echo "   CEK: Token Ngrok di start.sh valid atau Koneksi Internet Anda stabil."
        echo "--------------------------------------------------------"
        exit 1
    fi
